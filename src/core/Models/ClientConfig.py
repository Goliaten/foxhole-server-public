...
class GlobalShardConfigModel(BaseModel):
    defaultShardId: int
    bEnableOverpopMode: bool
    bRedirectToShardSelector: bool
    overpopShardId: int
...
class AvailableShardModel(BaseModel):
    normalizedGlobalPopulation: float
    descriptionType: Optional[str]
    shardName: str
    shardId: int
...
class ClientConfigModel(BaseModel):
    globalShardConfig: GlobalShardConfigModel
    availableShardList: List[AvailableShardModel]
